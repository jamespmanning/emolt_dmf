...
#CLEV   TOWR  	RCKY	MNMT	ENDC	MARS	RMNC	Sipp	Sculpin	 Scortons CUTTY
...
inputfile='DMF Temp Update ERRDAP 2023_time.xlsx' # file sent by ALex B in jan 2023
...
for k in range(numsites): # if you want specific one like "RCKY" specify "k in [2]:", for example
    print(sites[k])
    ...
    try:
        ...
        for kk in range(len(tsdf)):
            if type(tsdf['Time'][kk])==np.float64:
                hr.append(tsdf['Time'][kk])
            else: # case where it is formatted as "time"
